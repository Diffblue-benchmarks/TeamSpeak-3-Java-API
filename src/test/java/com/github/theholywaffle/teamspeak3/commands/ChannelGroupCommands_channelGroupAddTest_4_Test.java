...
public class ChannelGroupCommands_channelGroupAddTest_4_Test {
  @Test
  public void channelGroupAddTest() throws Exception {
    // Arrange
    String groupName = "aaaaa";
    PermissionGroupDatabaseType type = PermissionGroupDatabaseType.TEMPLATE;

    // Act
    Command actual = ChannelGroupCommands.channelGroupAdd(groupName, type);

    // Assert
    String name = actual.getName();
    String toStringResult = actual.toString();
    CommandFuture<DefaultArrayResponse> future = actual.getFuture();
    Assert.assertEquals("channelgroupadd", name);
    ...
    Assert.assertEquals("channelgroupadd name=aaaaa type=0", toStringResult);
