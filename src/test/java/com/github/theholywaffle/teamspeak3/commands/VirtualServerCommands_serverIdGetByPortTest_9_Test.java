...
public class VirtualServerCommands_serverIdGetByPortTest_9_Test {
  @Test
  public void serverIdGetByPortTest() throws Exception {
    // Arrange
    int port = 1;

    // Act
    Command actual = VirtualServerCommands.serverIdGetByPort(port);

    // Assert
    String name = actual.getName();
    String toStringResult = actual.toString();
    CommandFuture<DefaultArrayResponse> future = actual.getFuture();
    Assert.assertEquals("serveridgetbyport", name);
    ...
    Assert.assertEquals("serveridgetbyport virtualserver_port=1", toStringResult);
