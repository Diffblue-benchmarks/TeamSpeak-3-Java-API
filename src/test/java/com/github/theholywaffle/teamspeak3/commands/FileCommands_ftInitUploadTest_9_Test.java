...
public class FileCommands_ftInitUploadTest_9_Test {
  @Test
  public void ftInitUploadTest() throws Exception {
    // Arrange
    int transferId = 1;
    String path = "aaaaa";
    int channelId = 1;
    ...
    // Act
    Command actual = FileCommands.ftInitUpload(transferId, path, channelId, channelPassword, size, overwrite);

    // Assert
    String name = actual.getName();
    String toStringResult = actual.toString();
    CommandFuture<DefaultArrayResponse> future = actual.getFuture();
    Assert.assertEquals("ftinitupload", name);
    ...
    Assert.assertEquals("ftinitupload clientftfid=1 name=\\/aaaaa cid=1 cpw=aaaaa size=1 overwrite=1 resume=0 proto=0",
        toStringResult);
