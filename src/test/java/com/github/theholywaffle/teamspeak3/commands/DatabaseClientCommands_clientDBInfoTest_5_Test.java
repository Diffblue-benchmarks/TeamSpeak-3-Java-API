...
public class DatabaseClientCommands_clientDBInfoTest_5_Test {
  @Test
  public void clientDBInfoTest() throws Exception {
    // Arrange
    int clientDBId = 1;

    // Act
    Command actual = DatabaseClientCommands.clientDBInfo(clientDBId);

    // Assert
    String name = actual.getName();
    String toStringResult = actual.toString();
    CommandFuture<DefaultArrayResponse> future = actual.getFuture();
    Assert.assertEquals("clientdbinfo", name);
    ...
    Assert.assertEquals("clientdbinfo cldbid=1", toStringResult);
