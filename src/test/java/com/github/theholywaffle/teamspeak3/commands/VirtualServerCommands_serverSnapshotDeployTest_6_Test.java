...
public class VirtualServerCommands_serverSnapshotDeployTest_6_Test {
  @Test
  public void serverSnapshotDeployTest() throws Exception {
    // Arrange
    String snapshot = "aaaaa";

    // Act
    Command actual = VirtualServerCommands.serverSnapshotDeploy(snapshot);

    // Assert
    String name = actual.getName();
    String toStringResult = actual.toString();
    CommandFuture<DefaultArrayResponse> future = actual.getFuture();
    Assert.assertEquals("serversnapshotdeploy", name);
    ...
    Assert.assertEquals("serversnapshotdeploy aaaaa", toStringResult);
