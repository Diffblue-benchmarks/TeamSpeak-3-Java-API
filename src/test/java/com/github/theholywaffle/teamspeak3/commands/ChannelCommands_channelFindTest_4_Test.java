...
public class ChannelCommands_channelFindTest_4_Test {
  @Test
  public void channelFindTest() throws Exception {
    // Arrange
    String pattern = "aaaaa";

    // Act
    Command actual = ChannelCommands.channelFind(pattern);

    // Assert
    String name = actual.getName();
    String toStringResult = actual.toString();
    CommandFuture<DefaultArrayResponse> future = actual.getFuture();
    Assert.assertEquals("channelfind", name);
    ...
    Assert.assertEquals("channelfind pattern=aaaaa", toStringResult);
