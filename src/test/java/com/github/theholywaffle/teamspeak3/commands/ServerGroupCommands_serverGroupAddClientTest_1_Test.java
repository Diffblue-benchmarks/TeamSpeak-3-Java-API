...
public class ServerGroupCommands_serverGroupAddClientTest_1_Test {
  @Test
  public void serverGroupAddClientTest() throws Exception {
    // Arrange
    int groupId = 1;
    int clientDBId = 1;

    // Act
    Command actual = ServerGroupCommands.serverGroupAddClient(groupId, clientDBId);

    // Assert
    String name = actual.getName();
    String toStringResult = actual.toString();
    CommandFuture<DefaultArrayResponse> future = actual.getFuture();
    Assert.assertEquals("servergroupaddclient", name);
    ...
    Assert.assertEquals("servergroupaddclient sgid=1 cldbid=1", toStringResult);
