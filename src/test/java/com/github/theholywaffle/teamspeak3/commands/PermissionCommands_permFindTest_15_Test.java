...
public class PermissionCommands_permFindTest_15_Test {
  @Test
  public void permFindTest() throws Exception {
    // Arrange
    String permName = "aaaaa";

    // Act
    Command actual = PermissionCommands.permFind(permName);

    // Assert
    String name = actual.getName();
    String toStringResult = actual.toString();
    CommandFuture<DefaultArrayResponse> future = actual.getFuture();
    Assert.assertEquals("permfind", name);
    ...
    Assert.assertEquals("permfind permsid=aaaaa", toStringResult);
